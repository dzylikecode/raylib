...
@Deprecated('Use start + (end - start) * amount instead')
double Lerp(double start, double end, double amount) =>
    start + (end - start) * amount;

@Deprecated('Use (value - start) / (end - start) instead')
...
bool FloatEquals(double x, double y) =>
    (x - y).abs() <= 1e-6 * math.max(1.0, math.max(x.abs(), y.abs()));
...
bool Vector2Equals(Vector2 p, Vector2 q) => (p - q).length2 < 1e-10;
...
Vector3 Vector3Min(Vector3 v1, Vector3 v2) => Vector3(
  math.min(v1.x, v2.x),
  math.min(v1.y, v2.y),
  math.min(v1.z, v2.z),
);

Vector3 Vector3Max(Vector3 v1, Vector3 v2) => Vector3(
  math.max(v1.x, v2.x),
  math.max(v1.y, v2.y),
  math.max(v1.z, v2.z),
);
...
Matrix4 MatrixAdd(Matrix4 left, Matrix4 right) {
  ...
  for (var i = 0; i < 16; i++) { r[i] = left[i] + right[i]; }
  return r;
}
...
Matrix4 MatrixRotateXYZ(Vector3 angle) =>
    Matrix4.rotationX(angle.x) * Matrix4.rotationY(angle.y) * Matrix4.rotationZ(angle.z);

Matrix4 MatrixRotateZYX(Vector3 angle) =>
    Matrix4.rotationZ(angle.z) * Matrix4.rotationY(angle.y) * Matrix4.rotationX(angle.x);
...
Quaternion QuaternionSlerp(Quaternion q1, Quaternion q2, double amount) {
  ...
  if (sinHalf.abs() < 0.001) {
    return Quaternion(
      q1.x * 0.5 + q2x * 0.5, q1.y * 0.5 + q2y * 0.5,
      q1.z * 0.5 + q2z * 0.5, q1.w * 0.5 + q2w * 0.5,
    );
  }
  final ra = math.sin((1 - amount) * half) / sinHalf;
  final rb = math.sin(amount * half) / sinHalf;
  return Quaternion(
    q1.x * ra + q2x * rb, q1.y * ra + q2y * rb,
    q1.z * ra + q2z * rb, q1.w * ra + q2w * rb,
  );
}
...
Quaternion QuaternionFromMatrix(Matrix4 mat) {
  ...
  if (trace > 0) {
    ...
    return Quaternion(0.25 * s, (mat[4] + mat[1]) / s, (mat[8] + mat[2]) / s, (mat[6] - mat[9]) / s);
  } else if (mat[5] > mat[10]) {
    final s = 2.0 * math.sqrt(1.0 + mat[5] - mat[0] - mat[10]);
    return Quaternion((mat[4] + mat[1]) / s, 0.25 * s, (mat[9] + mat[6]) / s, (mat[8] - mat[2]) / s);
  } else {
    final s = 2.0 * math.sqrt(1.0 + mat[10] - mat[0] - mat[5]);
    return Quaternion((mat[8] + mat[2]) / s, (mat[9] + mat[6]) / s, 0.25 * s, (mat[1] - mat[4]) / s);
  }
}
...
@Deprecated('Use (p.x - q.x).abs() + ... < epsilon instead')
bool QuaternionEquals(Quaternion p, Quaternion q) =>
    (p.x - q.x).abs() + (p.y - q.y).abs() + (p.z - q.z).abs() + (p.w - q.w).abs() < 1e-6;
