...
int main() {
  ...
  // Set axis deadzones
  const double leftStickDeadzoneX = 0.1;
  const double leftStickDeadzoneY = 0.1;
  const double rightStickDeadzoneX = 0.1;
  const double rightStickDeadzoneY = 0.1;
  const double leftTriggerDeadzone = -0.9;
  const double rightTriggerDeadzone = -0.9;

  Rectangle vibrateButton = .zero();
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    vibrateButton = .fromLTWH(
      10,
      70 + 20 * GetGamepadAxisCount(gamepad) + 20,
      75,
      24,
    );
    ...
    if (IsGamepadAvailable(gamepad)) {
      ...
      // Get axis values
      double leftStickX = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_LEFT_X);
      double leftStickY = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_LEFT_Y);
      double rightStickX = GetGamepadAxisMovement(
        gamepad,
        GAMEPAD_AXIS_RIGHT_X,
      );
      double rightStickY = GetGamepadAxisMovement(
        gamepad,
        GAMEPAD_AXIS_RIGHT_Y,
      );
      double leftTrigger = GetGamepadAxisMovement(
        gamepad,
        GAMEPAD_AXIS_LEFT_TRIGGER,
      );
      double rightTrigger = GetGamepadAxisMovement(
        gamepad,
      ...
      if ((TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_1) >
              -1) ||
          (TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_2) >
              -1)) {
        ...
        DrawCircle(
          259 + (leftStickX * 20).toInt(),
          152 + (leftStickY * 20).toInt(),
          25,
          leftGamepadColor,
        );
        ...
        DrawCircle(
          461 + (rightStickX * 20).toInt(),
          237 + (rightStickY * 20).toInt(),
          25,
          rightGamepadColor,
        );
      ...
      } else if (TextFindIndex(TextToLower(GetGamepadName(gamepad)), PS_ALIAS) >
          -1) {
        ...
        DrawCircle(
          319 + (leftStickX * 20).toInt(),
          255 + (leftStickY * 20).toInt(),
          25,
          leftGamepadColor,
        );
        ...
        DrawCircle(
          475 + (rightStickX * 20).toInt(),
          255 + (rightStickY * 20).toInt(),
          25,
          rightGamepadColor,
        );
        ...
        DrawRectangleRounded(.fromLTWH(175, 110, 460, 220), 0.3, 16, DARKGRAY);
        ...
        // Draw buttons: left-right back
        DrawRectangleRounded(.fromLTWH(215, 98, 100, 10), 0.5, 16, DARKGRAY);
        DrawRectangleRounded(.fromLTWH(495, 98, 100, 10), 0.5, 16, DARKGRAY);
        if (IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_TRIGGER_1))
          DrawRectangleRounded(.fromLTWH(215, 98, 100, 10), 0.5, 16, RED);
        if (IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_TRIGGER_1))
          DrawRectangleRounded(.fromLTWH(495, 98, 100, 10), 0.5, 16, RED);
        ...
        DrawCircle(
          345 + (leftStickX * 20).toInt(),
          260 + (leftStickY * 20).toInt(),
          25,
          leftGamepadColor,
        );
        ...
        DrawCircle(
          465 + (rightStickX * 20).toInt(),
          260 + (rightStickY * 20).toInt(),
          25,
          rightGamepadColor,
        );
        ...
        DrawRectangle(
          ...
          110,
          15,
          (((1 + rightTrigger) / 2) * 70).toInt(),
          RED,
        );
      }

      DrawText(
        "DETECTED AXIS [${GetGamepadAxisCount(gamepad)}]:",
        10,
        50,
        10,
        MAROON,
      ...
      for (int i = 0; i < GetGamepadAxisCount(gamepad); i++) {
        DrawText(
          "AXIS $i: ${GetGamepadAxisMovement(gamepad, GamepadAxis.values[i]).toStringAsPrecision(2)}",
          20,
          70 + 20 * i,
          10,
          DARKGRAY,
      ...
      DrawText(
        "VIBRATE",
        vibrateButton.x.toInt() + 14,
        vibrateButton.y.toInt() + 1,
        10,
        DARKGRAY,
      );
      ...
      if (GetGamepadButtonPressed() != GAMEPAD_BUTTON_UNKNOWN)
        DrawText(
          "DETECTED BUTTON: ${GetGamepadButtonPressed()}",
          10,
          430,
          10,
          RED,
