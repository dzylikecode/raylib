...
int main()
{
    ...
    Rectangle touchArea = .fromLTWH(220, 10, screenWidth - 230.0.f, screenHeight - 20.0.f);

    int gesturesCount = 0;
    List<String> gestureStrings = List.generate(MAX_GESTURE_STRINGS, (i) => "\0");
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (CheckCollisionPointRec(touchPosition, touchArea) && (currentGesture != GESTURE_NONE))
        {
            if (currentGesture != lastGesture)
            {
                ...
                {
                    for (int i = 0; i < MAX_GESTURE_STRINGS; i++) gestureStrings[i] = "\0";
        ...
        BeginDrawing();
            ...
            DrawText("GESTURES TEST AREA", screenWidth - 270, screenHeight - 40, 20, Fade(GRAY, 0.5.f));

            for (int i = 0; i < gesturesCount; i++)
            {
                if (i%2 == 0) DrawRectangle(10, 30 + 20*i, 200, 20, Fade(LIGHTGRAY, 0.5.f));
                else DrawRectangle(10, 30 + 20*i, 200, 20, Fade(LIGHTGRAY, 0.3.f));
